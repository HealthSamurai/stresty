...
(zrf/reg-sub
  search-value
  (fn [[_ opts]]
    (zrf/subscribe [:zf.core/state opts]))
  (fn [state _]
...
(zrf/reg-sub
  options
  (fn [[_ opts]]
    (zrf/subscribe [:zf.core/schema opts]))
  (fn [schema _]
    (get schema :options)))
...
(zrf/reg-event-fx
  ...
  (fn [{:keys [db] ::keys [current-options]} [_ opts]]
    (let [value (zf/value db opts)
          {:keys [id-fn] :or {id-fn default-id-fn}} (zf/schema db opts)]
      {:dispatch-n [[activate opts (or (when value (find-option id-fn current-options value))
  ...
  (fn [{:keys [db]} [_ {:keys [opts data]}]]
    (let [state (zf/state db opts)]
      {:db         (-> db
       ...
       :dispatch-n [(when (nil? (:active state))
                      [activate opts (first data)])]})))
  ...
  (fn [{:keys [db]} [_ opts search-value]]
    (when-let [http (:http (zf/schema db opts))]
      {:db         (zf/set-state db opts [:loading] true)
       :http/fetch (merge (update http :params merge {(:q http :q) search-value})
                          {:unbundle true
...
(zrf/reg-event-fx
  ...
  (fn [{:keys [db]} [_ opts value]]
    (let [schema (zf/schema db opts)]
      {:focus      (zf/get-id opts :input)
       :dispatch-n [[close opts]
                    [::zf/set-value opts (some->> value (value-fn (:value-fn schema)))]
                    (when-let [on-change (:on-change schema)]
                      [(:event on-change) (assoc on-change :opts opts :value value)])]})))
...
(defn zf-select
  ...
  (let [should-scroll           (zrf/subscribe [should-scroll opts])
        ...
        value                   (zrf/subscribe [:zf.core/value opts])
        schema                  (zrf/subscribe [:zf.core/schema opts])
