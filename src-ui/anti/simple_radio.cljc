...
(defn radio
  [props]
  (let [i [:input (merge (dissoc props :class)
...
(defn zf-radio
  ...
  [radio
   (merge
     (dissoc props :opts)
     {:checked   (or @(zrf/subscribe [:zf.core/value (:opts props)]) false)
      :on-change #(zrf/dispatch [:zf.core/set-value (:opts props) (.. % -target -checked)])})])
...
(defn zf-radio-group
  ...
  [radio-group
   (merge
     (dissoc props :opts)
     {:value     (or @(zrf/subscribe [:zf.core/value (:opts props)]) [])
      :on-change #(zrf/dispatch [:zf.core/set-value (:opts props) %])
      :options   (:options @(zrf/subscribe [:zf.core/schema (:opts props)]))})])
